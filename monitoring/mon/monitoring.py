...
# Aggiungi la directory padre al path di ricerca dei moduli
...
class Check:
    ...
    def ping(self):
        from bot_Discord.bot import notifier
        result = ping(self.ip, timeout=2)

        with open(hosts_path, 'r') as f:
                data = json.load(f)
        unreachable_hosts = data.get('unreachable_hosts', {})

        if result is not None:
            print(f"{self.hosts} ({self.ip}): Raggiungibile ✅")

            if self.hosts in unreachable_hosts:
                del unreachable_hosts[self.hosts]
                data['unreachable_hosts'] = unreachable_hosts

                with open(hosts_path, 'w') as f:
                    json.dump(data, f, indent=4)

                log_message = f"{self.hosts} ({self.ip}): È tornato online ✅"
                logger.info(log_message)

                # Prepara il messaggio completo per Discord
                timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                full_message = f"[{this_machine_hostname}] {timestamp} [INFO] {log_message}"

                if notifier:
                    notifier.notify_info(self.hosts, full_message)
        else:
            log_message = f"{self.hosts} ({self.ip}): Non raggiungibile ❌"
            logger.error(log_message)

            if self.hosts not in unreachable_hosts:
                unreachable_hosts[self.hosts] = {
                    "ip": self.ip,
            ...
            # Prepara il messaggio completo per Discord
            timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            full_message = f"[{this_machine_hostname}] {timestamp} [ERROR] {log_message}"


            if notifier:
                notifier.notify_error(self.hosts, full_message)
