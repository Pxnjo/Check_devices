import requests, json, os , sys, threading, asyncio
import time, pprint
...
def api_request(ip_addresses, hosts, this_device_ip):
    try:
        ...
        response = requests.post(
            ...
            verify = ca_cert_path,  # Path al certificato CA
        )
        ...
        if response.status_code == 200:
            ...
            income_to_monitor = response.json().get('to_monitor', {})
            ...
            local_to_monitor = local_hosts_data.get('to_monitor', {})
            flask_not_responding = local_hosts_data.get('flask_not_responding', {})
            unreachable_hosts = local_hosts_data.get('unreachable_hosts', {})

            # Se ci sono host a cui prima il server non rispondeva, toglierli siccome hanno risposto
            ...
            for hostname, ip in data.items():
                if hostname not in hosts and ip not in this_device_ip.values() and hostname not in combined_forgot:# Se l'host non è sè stesso o da eliminare o non è gia presente lo salva
                    if bot.notifier is not None:
                        asyncio.run_coroutine_threadsafe(bot.create_channels(hostname), bot.notifier.client.loop) # Crea canale Discord
                    hosts[hostname] = ip
                    has_changes = True

            # Aggiorna to_monitor solo se ci sono nuovi host
            for hostname, ip in income_to_monitor.items():
                if hostname not in local_to_monitor and ip not in this_device_ip.values() and hostname not in combined_forgot:# Se l'host non è sè stesso o da eliminare o non è gia presente lo salva
                    if bot.notifier is not None:
                        asyncio.run_coroutine_threadsafe(bot.create_channels(hostname), bot.notifier.client.loop) # Crea canale Discord
                    local_to_monitor[hostname] = ip
                    has_changes = True

            # Rimuovi dai dizionari gli hosts presenti in combined_forgot
            if combined_forgot:
                for hostname in combined_forgot.keys():
                    if hostname in hosts: # Se l'host è in hosts, rimuovilo
                        del hosts[hostname]
                        has_changes = True
                    elif hostname in local_to_monitor: # Se l'host è in local_to_monitor, rimuovilo
                        del local_to_monitor[hostname]
                        has_changes = True
                    elif hostname in unreachable_hosts: # Se l'host è in unreachable_hosts, rimuovilo
                        del unreachable_hosts[hostname]
                        has_changes = True

            # Scrivi su file solo se ci sono stati cambiamenti
            if has_changes:
                manage_file(hosts_path, 'w', {'hosts': hosts, 'this_device_ip': this_device_ip, 'to_monitor': local_to_monitor})
...
def update_hosts_from_api():
    ...
    try:
        ...
        my_ips = set(this_device_ip.values())
        # Ciclo sui known hosts
        for hostname, ip_addresses in hosts.copy().items():
            if ip_addresses not in my_ips:
                api_request(ip_addresses, hosts, this_device_ip)

        # Ciclo anche sugli unknown hosts, con lo stesso filtro
        if unknown_hosts:
...
if __name__ == '__main__':
    ...
    try:
        ...
        updater.stop()
        print("Aggiornamento interrotto.")